...
function parsePage(page) {
  var page_url;
  if (page[0][0] !== undefined) {
    page_url = page[0][0].page_url;
  }
  else {
    page_url = "Error During Processing"
  }
  var total_obj = 0;
  var CDNCounter = 0;
  var total_xcache = 0; 
...
for (var i = 0; i < data.length; i++) {
   const page = data[i];
   // const header = page[0][1];
   // console.log(Object.keys(page[0][0]))
   // console.log(typeof(header));
   parsePage(page);
}

fs.writeFileSync(fileName, JSON.stringify(all_pages), (err) => {
  if(err) {
